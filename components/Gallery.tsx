...
import type { Chair, EraFilter } from '@/types'
import ChairCard from './ChairCard'

const ChairModal = dynamic(() => import('./ChairModal'), { ssr: false })
  ...
  return (
    <main className="flex-1 px-4 py-12 sm:px-6 lg:px-8">
      <div className="mx-auto max-w-7xl space-y-10">
        ...
        {/* Filter bar */}
        <div role="tablist" aria-label="Filtrar por época" className="flex flex-wrap gap-2">
          {eras.map((era) => (
            <button
              key={era}
              role="tab"
        ...
        <p className="text-sm text-zinc-400">
          {filtered.length === 1 ? '1 silla' : `${filtered.length} sillas`}
        ...
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          {filtered.map((chair) => (
            <ChairCard key={chair.id} chair={chair} onClick={() => setSelected(chair)} />
          ))}
        </div>
      </div>
      ...
      {/* Modal */}
      {selected && (
        <ChairModal chair={selected} onClose={() => setSelected(null)} />
      )}
    </main>
  )
}
